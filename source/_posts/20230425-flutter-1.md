...
이전 글 [Chocolatey 설치](https://hgko1207.github.io/2021/07/21/chocolatey/)을 참고해서 Chocolatey 를 먼저 설치를 합니다.

## 2. Flutter 설치

관리자 권한으로 PowerShell 을 열고 아래 명령어를 입력해 설치를 합니다.

```shell
choco install flutter
```

설치가 완료되면 `flutter` 명령어를 입력하였을 때 아래와 같이 메시지가 나옵니다.
...
안드로이드 앱을 개발하려고 한다면 [Android setup](https://docs.flutter.dev/get-started/install/windows#android-setup) 사이트에 접속해서 가이드대로 Android 에뮬레이터를 설치하면 됩니다.
...
PowerShell 에서 `flutter doctor` 명령어를 입력해서 Flutter 가 설치된 Android Studio 를 찾았는지 확인합니다.
...
PowerShell 또는 명령 프롬프트에서 아래 명령어를 실행하여 라이언스 서명을 시작합니다.
...
`flutter doctor` 명령어를 입력하였을 때 Windows 10 SDK 이 없다고 하는 메시지가 출력될 경우 해결 방법입니다.
