...
동네예보 정보를 조회하기 위해 [공공데이터 포털](https://www.data.go.kr/index.do) 사이트의 동네예보 조회서비스 API를 사용하였다.

## Open API 활용 방법

- [동네예보 조회서비스](https://www.data.go.kr/data/15057682/openapi.do) 사이트에서 활용신청을 하고 Service Key를 발급받는다.
- 오픈 API 활용 가이드를 다운로드 하여 조회 조건 및 결과 데이터를 이해한다.

## 동네예보 데이터 조회

```url
http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst
  ?ServiceKey={발급받은 Service Key}
  &pageNo=1&numOfRows=10&dataType=JSON
  &base_date={현재날짜}&base_time={현재시간}
  &nx={지점 X 좌표값}&ny={지점 Y 좌표값}
```
